var _is_Date_Time = true;
function setDate(name)
{
	_is_Date_Time = false;
	var ob = document.getElementsByName(name)[0];
	calendar(ob);
}
...
document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=144 height=193></iframe></div>");
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.timeShow   = _is_Date_Time;        //是否返回时间
    this.drag       = true;         //是否允许拖动
    this.darkColor  = "#85A063";    //控件的暗色


    this.lightColor = "#FFFFFF";    //控件的亮色


    this.btnBgColor = "#C3CFE2";    //控件的按钮背景色
...
function calendar() //主调函数
{
    ...
    if  (!_is_Date_Time) WebCalendar.dateReg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})$/;
